        ...
        for i = 1:len - 1
            ...
            for j = i + 1:len
                ...
                if distance² < dist²
                    ...
                    #end
                    ccnt += 1
                    cache[ccnt, threadx, thready, threadz] = minmax(indi, indj)
                    if ccnt == cs
...
"""
    ...
    ∑∇W_3d!(∑∇W, ∇W, pairs, points, kernel, H⁻¹) 

Compute gradients.

"""
function ∑∇W_3d!(∑∇W, ∇W, pairs, points, kernel, H⁻¹) 
...
function kernel_∑∇W_3d!(∑∇W, ∇W, pairs, points, kernel, H⁻¹) 
    index = (blockIdx().x - Int32(1)) * blockDim().x + threadIdx().x
    if index <= length(pairs)
        pair  = pairs[index]
        pᵢ    = pair[1]; pⱼ = pair[2]
        if pᵢ != 0
            ...
            ∇Wᵢⱼ  = (Δx[1] * dwk_r, Δx[2] * dwk_r, Δx[3] * dwk_r)

            ∑∇Wˣ = ∑∇W[1]
            ∑∇Wʸ = ∑∇W[2]
            ∑∇Wᶻ = ∑∇W[3]
            CUDA.@atomic ∑∇Wˣ[pᵢ] += ∇Wᵢⱼ[1]
            CUDA.@atomic ∑∇Wʸ[pᵢ] += ∇Wᵢⱼ[2]
            CUDA.@atomic ∑∇Wᶻ[pᵢ] += ∇Wᵢⱼ[3]

            CUDA.@atomic ∑∇Wˣ[pⱼ] -= ∇Wᵢⱼ[1]
            CUDA.@atomic ∑∇Wʸ[pⱼ] -= ∇Wᵢⱼ[2]
            CUDA.@atomic ∑∇Wᶻ[pⱼ] -= ∇Wᵢⱼ[3]
            ∇W[index] = ∇Wᵢⱼ
        end
    end
    return nothing
